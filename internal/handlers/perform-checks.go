...
func (repo *DBRepo) TestCheck(w http.ResponseWriter, r *http.Request) {
	...
	if newStatus != hs.Status {
		...
		mailMessage := channeldata.MailData{
			ToAddress: "karalar@example.com", // Replace with actual recipient email
			Subject:   "Service Status Changed",
			Content: template.HTML(fmt.Sprintf("Service %s has changed status from %s to %s. Message: %s",
				hs.Service.ServiceName, oldStatus, newStatus, msg)),
		}
